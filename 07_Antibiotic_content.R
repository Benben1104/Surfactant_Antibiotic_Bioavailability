...
residuals <- (aov(periplasmic ~ group, data = d_CEF_DTAC))$residuals
shapiro.test(residuals)
bartlett.test(periplasmic ~ group, data = d_CEF_DTAC)
aov.1 <- aov(periplasmic ~ group, data = d_CEF_DTAC)
summary(aov.1)
TukeyHSD(aov.1)

ggplot(d_CEF_DTAC, aes(group, periplasmic, color = group))+
  ...
  labs(x = NULL,
       y = expression("Periplasmic CEF (ng"~cell^"-1"*")"),
       title = expression("DTAC+CEF"))+
...
ggsave("DTAC+CEF.png", width = 650/90, height = 420/90, dpi = 600, unit = "in")
...
residuals <- (aov(periplasmic ~ group, data = d_CEF_SDS))$residuals
shapiro.test(residuals)
bartlett.test(periplasmic ~ group, data = d_CEF_SDS)
aov.2 <- aov(periplasmic ~ group, data = d_CEF_SDS)
summary(aov.2)
TukeyHSD(aov.2)

ggplot(d_CEF_SDS, aes(group, periplasmic, color = group))+
  geom_point(stat = "summary", fun = "mean", alpha = 1.0, size = 9)+
  geom_errorbar(stat = "summary",
                fun.min = function(x) mean(x)-sd(x),
                fun.max = function(x) mean(x)+sd(x),
...
ggsave("SDS+CEF.png", width = 650/90, height = 420/90, dpi = 600, unit = "in")
...
residuals <- (aov(periplasmic ~ group, data = d_CEF_TX))$residuals
shapiro.test(residuals)
bartlett.test(periplasmic ~ group, data = d_CEF_TX)
aov.3 <- aov(periplasmic ~ group, data = d_CEF_TX)
summary(aov.3)
TukeyHSD(aov.3)

ggplot(d_CEF_TX, aes(group, periplasmic, color = group))+
  geom_point(stat = "summary", fun = "mean", alpha = 1.0, size = 9)+
  geom_errorbar(stat = "summary",
                fun.min = function(x) mean(x)-sd(x),
                fun.max = function(x) mean(x)+sd(x),
...
ggsave("TX-100+CEF.png", width = 650/90, height = 420/90, dpi = 600, unit = "in")
...
ggsave("DTAC+TET.png", width = 650/90, height = 420/90, dpi = 600, unit = "in")
...
ggsave("SDS+TET.png", width = 650/90, height = 420/90, dpi = 600, unit = "in")
...
ggsave("TX-100+TET.png", width = 650/90, height = 420/90, dpi = 600, unit = "in")
